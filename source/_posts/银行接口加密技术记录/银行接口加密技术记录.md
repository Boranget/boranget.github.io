...
2. 发送方使用私钥（类型 RSA 或 DSA）对摘要进行加密。这是实际签名步骤。

    在发送方的 PGP 密钥环中查找私钥。

3. 加密的哈希值以及已使用的哈希算法已写入签名元素，该签名元素将与有效负载一起（作为 PGP 签名格式）发送到接收方。私钥签名人的密钥标识也写成 PGP 签名格式。

4. 接收方获取 PGP 签名格式。

5. 接收方从签名中选择密钥标识，并使用该密钥标识在接收方的 PGP 公钥环中查找正确的公钥。这是与用于签名有效负载的私钥相对应的公钥。
...
5. 发送方将加密的对称密钥和密钥标识写入消息的加密信息元素。

    密钥标识用于标识用于加密的公钥（因为 PGP 公钥环可以包含多个公钥）。

    加密信息元素与加密的有效负载一起发送到接收方。

6. 接收方获取消息，并根据密钥标识（在加密信息元素中）在 PGP 密钥环中查找正确的私钥（与用于加密有效负载的公钥相关联）。
